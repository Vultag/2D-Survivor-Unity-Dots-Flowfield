...
public partial struct FlowfieldSystem : ISystem
{
    ...
    public void OnCreate(ref SystemState state)
    {
        ...
        TargetQuery = state.EntityManager.CreateEntityQuery(typeof(TargetData));
    }
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        updatedTargetCell = new int2(Mathf.Clamp(updatedTargetCell.x,0,gridSize.x-1), Mathf.Clamp(updatedTargetCell.y, 0, gridSize.y-1));
    ...
    private void RebuildGrid(ref SystemState state)
    {
        ...
        ushort cellXresolution = (ushort)Mathf.CeilToInt(dimentions.x + 2);
        ushort cellYresolution = (ushort)Mathf.CeilToInt(dimentions.y + 2);
        ...
        FlowfieldGridStorage.flowfieldGrid = new NativeArray<FlowfieldCellData>(cellXresolution* cellYresolution, Allocator.Persistent);
        flowfieldGrid = FlowfieldGridStorage.flowfieldGrid;
        flowfieldGridGPU = new NativeArray<FlowfieldCellGPU>(cellXresolution * cellYresolution, Allocator.Persistent);

        gridSize = new int2(cellXresolution, cellYresolution);
        FlowfieldGridStorage.gridSize = gridSize;
        Shader.SetGlobalVector("_FlowfieldPosSize", new Vector4(gridCenter.x, gridCenter.y, dimentions.x, dimentions.y));
        ...
        {
            Position = new float3(gridCenter.x, gridCenter.y, 0),
            Rotation = Quaternion.identity,
            Scale = 1
        });
        state.EntityManager.SetComponentData<PostTransformMatrix>(flowfieldE, new PostTransformMatrix
        {
            Value = float4x4.Scale(new float3(dimentions.x, dimentions.y, 1))
        });

        float cellsize = (dimentions.x / cellXresolution);
        cellNum = (ushort)(cellXresolution * cellYresolution);

        /// Test each cells against static bodies
        /// EXPENSIVE
        for (int i = 0; i < cellNum; i++)
        {
            int xIdx = i % cellXresolution;
            int yIdx = i / cellXresolution;

            Vector2 cellAABBlowerBound = new Vector2((gridCenter.x - dimentions.x*0.5f) + (cellsize * xIdx), (gridCenter.y - dimentions.y * 0.5f) + (cellsize * yIdx));

            //TreeInsersionSystem.DrawQuad(cellAABBlowerBound, new Vector2(cellAABBlowerBound.x + cellsize, cellAABBlowerBound.y + cellsize), Color.green);
            ...
            if(cellOnObstacle)
            {
                flowfieldGrid[i] = new FlowfieldCellData
                {
                    IsBlocked = true,
                    Direction = new float2(0.00001f, 0),
                    Cost = ushort.MaxValue,
                };
                flowfieldGridGPU[i] = new FlowfieldCellGPU
                {
                    IsBlocked = 1,
                    Cost = ushort.MaxValue,
                };
                ...
                ///Left
                newCell = flowfieldGrid[i - 1];
                newCell.IsNextToObstacle = true;
                flowfieldGrid[i - 1] = newCell;
                ///Up
                newCell = flowfieldGrid[i + cellXresolution];
                newCell.IsNextToObstacle = true;
                flowfieldGrid[i + cellXresolution] = newCell;
                ///Right
                newCell = flowfieldGrid[i + 1];
                newCell.IsNextToObstacle = true;
                flowfieldGrid[i + 1] = newCell;
            ...
            {
                FlowfieldCellData newCell = flowfieldGrid[i];
                newCell.InLineOfSight = true;
                newCell.Direction = new float2(0.00001f,0);
                flowfieldGrid[i] = newCell;
                FlowfieldCellGPU newGPUcell = flowfieldGridGPU[i];
                newGPUcell.InLineOfSight = 1;
                flowfieldGridGPU[i] = newGPUcell;
            }
        }
    ...
    private void UpdateGrid(ref SystemState state)
    {
        ...
        /// get the direction to the target here to input in gridCell for LOS 

        /// Reset the grid's direction and visible flag
        for (int i = 0; i < cellNum; i++)
        {
            FlowfieldCellData newCell = flowfieldGrid[i];
            FlowfieldCellGPU newCellGPU = flowfieldGridGPU[i];
            newCell.InLineOfSight = !newCell.IsBlocked;
            newCell.Direction = new float2(0.00001f, 0);
            newCell.Cost = (ushort)(ushort.MaxValue * newCellGPU.IsBlocked);
            newCellGPU.InLineOfSight = 1-newCellGPU.IsBlocked;
            newCellGPU.Cost = (ushort)(ushort.MaxValue * newCellGPU.IsBlocked);
            flowfieldGrid[i] = newCell;
            flowfieldGridGPU[i] = newCellGPU;
        }
        ...
        visitedCells = new NativeArray<bool>(flowfieldGrid.Length, Allocator.Temp);
    ...
    */
    ...
    {
        /// ADAPTE FOR 3 OTHER QUADRANT DIRECTION
        ...
        Vector2 relevantGridCorner = new Vector2(math.select(0, gridSize.x, cornerFromTraget.x > 0), math.select(0, gridSize.y, cornerFromTraget.y > 0));
        ...
        //while (evaluatedCoords.x < gridSize.x && evaluatedCoords.y< gridSize.y && evaluatedCoords.x >= 0 && evaluatedCoords.y >= 0)
        {
            ...
            //Debug.DrawLine(new Vector3(testcornerStart.x, testcornerStart.y, 0), new Vector3(testevaluatedPoint.x, testevaluatedPoint.y, 0), slope > slopeToGridCorner ? Color.green : Color.red, 3);
